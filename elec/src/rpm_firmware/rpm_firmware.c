...
#define RPM_TIMEOUT_US ((uint32_t)((60.0e6f / (MIN_RPM * (float)PPR))))
...
#define FRAME_MAX_BYTES ((((((PAYLOAD_LEN + CRC_LEN) * 8u * 6u + 4u) / 5u) + 16u) + 7u) / 8u)
...
static inline void pack_f32_le(uint8_t* dst, float f) { memcpy(dst, &f, 4); }
...
static void configure_dma(void) {
    ...
    dma_channel_configure(data_chan, &c, &spi_get_hw(SPI_PORT)->dr, frame_buf, 0, false);
}
...
static void check_rpms_zero() {
    ...
    bool is_zero_l = (motor_l_rpm > 0.0f) && ((uint32_t)(now - last_rise_l_us) > RPM_TIMEOUT_US);
    bool is_zero_r = (motor_r_rpm > 0.0f) && ((uint32_t)(now - last_rise_r_us) > RPM_TIMEOUT_US);
    ...
    if (motor_l_rpm > 0.0f && (uint32_t)(now - last_rise_l_us) > RPM_TIMEOUT_US) {
        motor_l_rpm = 0.0f;
    }

    if (motor_r_rpm > 0.0f && (uint32_t)(now - last_rise_r_us) > RPM_TIMEOUT_US) {
        motor_r_rpm = 0.0f;
    }
